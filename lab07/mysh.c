...
/*#ifdef DBUG
	...
	while (fgets(line, BUFSIZ, stdin) != NULL) {
		...
		char **ret = tokenise(line, " :");
		pid = fork();
		if (pid == 0) {
			execute(ret, path, envp);
...
void execute(char **args, char **path, char **envp) {
	...
	if (strncmp(args[0], ".",1) == 0 || strncmp(args[0], "/",1) == 0) {
		...
		int i = 0;
		while(path[i]!=NULL){
			char *commandtemp = strdup(path[i]);
			strcat(commandtemp,"/");
			strcat(commandtemp,args[0]);
			if(isExecutable(commandtemp)){
			 command = strdup(commandtemp);
			 EXECUTABLE = 1;
			 break;
			}
			i++;
